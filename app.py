...
def load_records():
    ...
    if os.path.exists(RECORD_FILE):
        try:
            # 1. 모든 컬럼을 일단 문자열로 읽어서 NaN을 빈 문자열로 치환
            ...
            # 3. 데이터 타입 강제 고정 (이 부분이 에러 해결 핵심)
            # 체크박스 컬럼 -> 불리언(True/False)
            for col in ["브릭", "실수"]:
...
if page == "📊 기록":
    ...
    if st.button("➕ 새로운 경기 추가"):
        ...
        # 추가 시에도 타입 일치화
        new_row["NO."] = new_row["NO."].astype(int)
        for col in ["브릭", "실수"]: new_row[col] = new_row[col].astype(bool)
