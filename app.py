...
st.markdown("""
    ...
    /* 선(파랑) / 후(빨강) 등 특정 텍스트 색상 강조 */
    /* 주의: 브라우저 환경에 따라 :contains 가 작동하지 않을 수 있으나 구조는 유지합니다 */
    div.row-widget.stDataFrame div[role="grid"] div[role="row"] div:contains("선") { color: #0000ff !important; font-weight: bold; }
    div.row-widget.stDataFrame div[role="grid"] div[role="row"] div:contains("후") { color: #ff0000 !important; font-weight: bold; }
    div.row-widget.stDataFrame div[role="grid"] div[role="row"] div:contains("승") { color: #0000ff !important; font-weight: bold; }
    div.row-widget.stDataFrame div[role="grid"] div[role="row"] div:contains("패") { color: #ff0000 !important; font-weight: bold; }
...
if menu == "📊 기록":
    ...
    # [수정 포인트] 에러 해결을 위해 width를 숫자로 지정하고 정렬 설정 추가
    edited_df = st.data_editor(st.session_state.df, use_container_width=True, num_rows="dynamic", hide_index=True, key="main_editor",
        column_config={
            ...
            "내 덱": st.column_config.SelectboxColumn("내 덱", options=st.session_state.metadata["my_decks"], width=110, alignment="center"),
            "상대 덱": st.column_config.SelectboxColumn("상대 덱", options=st.session_state.metadata["opp_decks"], width=110, alignment="center"),
            "아키타입": st.column_config.SelectboxColumn("아키타입", options=st.session_state.metadata["archetypes"], width=90, alignment="center"),
            "승패 요인": st.column_config.SelectboxColumn("승패 요인", options=st.session_state.metadata["win_loss_reasons"], width=110, alignment="center"),
            ...
            "비고": st.column_config.TextColumn("비고", width=450) # 비고 칸 크게 확장
        })
    ...
    if not df.empty:
        ...
        st.markdown(render_styled_table(f"{m_my} vs {m_opp}", df[(df['내_덱']==m_my) & (df['상대_덱']==m_opp)]), unsafe_allow_html=True)
