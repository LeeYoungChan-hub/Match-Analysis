...
# ---------------------------------------------------------
    ...
    with m_col2:
        arche_str = st.text_area("상대 아키타입", ", ".join(st.session_state.metadata["archetypes"]), height=100)
        reasons_str = st.text_area("승패 요인 리스트", ", ".join(st.session_state.metadata["win_loss_reasons"]), height=100)
        cards_str = st.text_area("관심 카드 리스트", ", ".join(st.session_state.metadata["target_cards"]), height=100)
